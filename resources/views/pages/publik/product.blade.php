            ...
            <div class="grid grid-cols-2 px-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-1 lg:gap-6 mt-12">
                @forelse ($products as $item)
                    <div class="w-full px-4">
                        <div
                            class=" bg-gradient-to-r from-slate-300 via-gray-400 to-zinc-500 border border-gray-500  rounded-xl shadow-lg mb-10 overflow-hidden transition duration-300 ease-in-out hover:shadow-2xl hover:-translate-y-2">
                            <img class="w-full object-cover " src="{{ Storage::url('products/' . $item->image) }}" alt="{{ $item->name }}">
                            <div class="py-5 px-6">
                                <h3 class="mb-5 text-sm lg:text-lg text-gray-800 font-extrabold uppercase">{{ $item->name_product }}</h3>
